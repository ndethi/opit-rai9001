...
The knowledge graph serves as the structured semantic foundation for ontology-grounded retrieval, encoding the 959-concept ontology and 100-proverb corpus developed in Chapter~\ref{chap:methodology}.

\subsection{Neo4j Schema Design}
\label{subsec:neo4j_schema}

The graph schema implements a multi-layered structure with four node types: (1) \texttt{:Proverb} nodes (100 instances) store Kikuyu text, expert translation, cultural meaning, business relevance, and thematic category; (2) \texttt{:CulturalConcept} nodes (959 instances) represent abstract themes with definitions, significance, and hierarchy levels; (3) \texttt{:UsageContext} nodes (5 instances) encode appropriate usage scenarios; (4) \texttt{:MoralLesson} nodes (5 instances) capture ethical principles. Five relationship types link nodes: \texttt{:EXPRESSES\_CONCEPT} (1,895 instances: proverb $\rightarrow$ concept with salience scores), \texttt{:TEACHES\_LESSON} (67 instances: proverb $\rightarrow$ moral), \texttt{:USED\_IN} (39 instances: proverb $\rightarrow$ context), \texttt{:RELATES\_TO} (4,394 instances: concept $\leftrightarrow$ concept), and \texttt{:SUBSUMES} (50 instances: concept hierarchy). Uniqueness constraints on \texttt{proverb\_id} and \texttt{concept\_name} ensure data integrity, while property indexes on \texttt{cultural\_weight} and \texttt{hierarchy\_level} optimize query performance.
...
\begin{figure}[htbp]
\centering
\resizebox{0.75\textwidth}{!}{\input{figures/retrieval-pipeline}}
