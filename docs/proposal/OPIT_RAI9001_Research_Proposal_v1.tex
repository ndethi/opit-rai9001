...
\begin{itemize}
\item Sourcing existing collections of Kikuyu proverbs, such as ``1000 Kikuyu Proverbs'' \cite{gikandi2023kikuyu, kenyatta2023mount}, and a collection of 100 proverbs and wise sayings of the Gikuyu (Kenya) about money and wealth compiled by Margaret Wambere Ireri \cite{ireri2024gikuyu}, which will serve as a primary corpus for wealth and prosperity-related proverbs.
...
\scalebox{0.8}{
\begin{tikzpicture}[
  level 1/.style={level distance=2.5cm, sibling distance=5.0cm},
  level 2/.style={level distance=2.2cm, sibling distance=2.8cm},
  every node/.style={
    draw, 
    rounded corners=2pt, 
    fill=blue!8, 
...
\node[root] {Kikuyu Proverb\\Ontology}
  child {
    node[level1] {Proverb\\Entity}
    child {node[level2] {Text Forms\\(Original,\\Translation)}}
    child {
      node[level2] {Social\\Usage\\Context}
      child[grow=down] {node[level2] {Linguistic\\Features}}
      child[grow=down] {node[level2] {Thematic\\Classification}}
    }
  }
  child {
    node[level1] {Cultural\\Context}
    child {node[level2] {Historical\\Background}}
    child {node[level2] {Cultural\\Values}}
  }
  child {
    node[level1] {Semantic\\Relationships}
    child {node[level2] {Contextual\\Dependencies}}
    child {
      node[level2] {Inter-Proverb\\Relations}
      child[grow=down] {node[level2, fill=blue!4] {Synonymy\\Relations}}
      child[grow=down] {node[level2, fill=blue!4] {Temporal\\Context}}
    }
  };
\end{tikzpicture}
}
