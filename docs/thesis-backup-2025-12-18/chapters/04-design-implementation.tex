...
Four primary node types encode different knowledge facets. \paragraph{Proverb Nodes} (\texttt{:Proverb}, 100 instances) represent individual proverbs, storing not just text but the rich contextual metadata essential for cultural translation: \texttt{proverb\_id} (unique identifier like MW\_001), \texttt{kikuyu\_text} (original proverb), \texttt{expert\_translation} (gold-standard English), \texttt{expert\_cultural\_meaning} (anthropological interpretation), \texttt{expert\_business\_relevance} (modern applicability), \texttt{cultural\_weight} (0.0-1.0 importance score from expert assessment), and \texttt{thematic\_category} (primary cultural theme like ``Reciprocity'' or ``Patience''). This property-rich design lets retrieval queries filter by cultural significance, not just keyword matching.
...
Populating the knowledge graph from raw proverb data required solving a chicken-and-egg problem: we needed concepts to structure the graph, but extracting concepts required understanding proverbs in context. Our multi-stage ETL pipeline (\texttt{scripts/ontology\_builder.py}) addressed this through iterative refinement. \textbf{Data extraction} began simply—parsing Ireri's proverb collection from CSV, extracting Kikuyu text, expert translations, and cultural annotations. \textbf{Concept identification} proved trickier: keyword matching alone produced too many false positives (``muti'' matching both tree and medicine concepts indiscriminately), so we added LLM-assisted extraction (\texttt{extract\_ontology\_concepts\_with\_llm.py}) that used surrounding context for disambiguation. For proverb MW\_002 (``Andu ni indo''), this extracted ``Community'', ``SocialCapital'', and ``ValueHierarchy''—concepts keyword matching would have missed. \textbf{Relationship inference} created \texttt{:EXPRESSES\_CONCEPT} edges weighted by salience scores computed from term frequency in cultural meaning annotations; concepts scoring above 0.7 became primary retrieval themes. \textbf{Weight assignment} aggregated expert surveys—3 Kikuyu elders rated importance on 1-5 scales, which we normalized to 0.0-1.0. High-weight proverbs like MW\_002 (0.92) get prioritized during retrieval. Finally, \textbf{validation} via \texttt{scripts/ontology\_validator.py} checked that all proverbs linked to concepts, hierarchies stayed acyclic, and no orphaned nodes existed.
...
The next chapter (Chapter~\ref{ch:evaluation}) presents the empirical evaluation of this system, demonstrating through rigorous quantitative and qualitative analysis that ontology-grounded retrieval significantly improves both cultural authenticity and translation fidelity compared to baseline approaches.
