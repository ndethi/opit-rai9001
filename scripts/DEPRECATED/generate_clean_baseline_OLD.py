...
Generates ONE row per proverb with SEPARATED systems:
...
NO duplicates, NO confusion, NO mixing of systems.
"""
...
{{
    ...
    def generate_clean_baseline(self, gold_standard_file: str, max_proverbs: int = 50):
        ...
        # Save final results
        return self._save_final(results)
    ...
    def _save_final(self, results):
        ...
        output_file = output_dir / f"baseline_translations_clean_50proverbs_{timestamp}.csv"
...
def main():
    ...
    print("  4. Google Translate (Commercial Baseline - No Kikuyu Support)")
    print("\nOutput: ONE row per proverb, NO duplicates, NO confusion")
    print("="*80 + "\n")
    
    # Confirm
    response = input("Proceed with clean baseline generation? (y/n): ")
    if response.lower() != 'y':
        print("❌ Cancelled")
        return
    ...
    print("  3. Make foundation decision for OG-RAG development")
    print()
